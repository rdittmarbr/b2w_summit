...
function TcDesktop( aDebug ) {
  /*------------------=-----------=----------------------------------------------------------------------------
	...
	this.debug     = function ( aLog , aType , aDom ) {
		aLog = aLog || '', aType = aType || 0, aDom = aDom;
		var aName = getStack(1);
		// se for um vetor não exibe
		var aMsg = this.constructor.name +'.'+ aName;
		if( isDom(aDom) ) {
			if( aType == 0 ) {
			...
			if( aType == 0 ) {
				console.log( aMsg , aLog );
			} else if( aType == 1 ) {
				console.info( aMsg , aLog );
			} else if( aType == 2 ) {
				console.error( aMsg , aLog );
			} else {
				console.log( aMsg , aLog );
			}
		}
	...
	this.FDebug    = aDebug || false;             // Console debug
	this.FVideo    = false;                       // Armazena os recursos da Camera
	this.FSkin     = '';                          // Configurações do Skin
	this.FAnimated = true;                        // Habilita as animações baseadas em javascript
	...
	window.oebeforeunload = function(e)
	{
	    this.onUnload;
	}
...
TcDesktop.prototype.processModule = function( aModule , aData ) {
	...
	if( aData.js && aData.js.before ) {
		...
		for( i in aData.js.before ) {
			if(this.FDebug) this.debug('Executando comando JS ' + aData.js.before[i] );
	...
	if( aData.js && aData.js.after ) {
		...
		for( i in aData.js.after ) {
			if(this.FDebug) this.debug('Executando comando JS ' + aData.js.after[i] );
