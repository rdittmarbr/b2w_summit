...
class TcPackage
{
  ...
  function __construct($aOwner = '', &$aTcDB, &$aTcUser, &$aTcDebug = false)
  {
    ...
    if (ES_SYDEBUG and is_object($this->FDebug)) {
      ...
      //$this -> FReturn['URL'] = $_SERVER['HTTP_REFERER'];
    }

    //Se o retorno do post no debug estiver ativo, retorna via JSON
    if (ES_POSTDEBUG) {
      $this->FReturn['_POST'] = $_POST;
    }
  ...
  {
    if ($this->FLog[1]) $this->log(__METHOD__);
  ...
  /*------------------=-----------=----------------------------------------------------------------------------
	...
	- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	aType               :string     : Ordem de xecução do JS - before(antes dos modulos)/after (após os modulos)
	aJS                 :string     : Código JS a executar
	- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	-----------------------------------------------------------------------------------------------------------*/
  function addJS($aType, $aJS)
  {
  ...
  function getModule($aForm, $aType, $aValue = false, $aOperation = 0)
  {
    ...
    if (file_exists($aModule['path'])) {
      ...
      if (($aModule['cfg']['public'] == true and ($aModule['cfg']['registered'] == false or $this->FUser->active())) or  isset($_SESSION['modules'][$aForm])) {
        $this->FModule[$aForm] = [];
      ...
      if ($this->FLog[1]) $this->log(__METHOD__, "Módulo não encontrado ");
