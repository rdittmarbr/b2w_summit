...
Class TcDebug {
  ...
  function __construct( $aOwner = '' , $aConfig = false ) {
    ...
    for( $i = 0 ; $i<10 ; $i++ ) { $this -> FWrite[$i] = ( ES_SYDEBUGLEVEL >= $i ); }

    //Gravando o debug
    $this -> write( $this -> FOwner , __METHOD__ , 'System Debug Level = ' . ES_SYDEBUGLEVEL );
  ...
  function save( $aForce = false ) {
    ...
    //Arquivo permite escrita
    if( !(is_writable($this -> FConfig['file']) ) ) {
      if($aForce) print('Erro ao salvar o log!');
      return false;
    }
    ...
    if( ! $this -> FWrited ) {
      ...
      fwrite($aFile , "IP : " . $this -> FConfig['ip'] . "(".ES_SYDEBUGLEVEL.")" . ES_SYTEXTEOF );
      if ( $this -> FConfig['config'] ) {
        fwrite($aFile, str_pad('',60,'-') . ES_SYTEXTEOF );
        fwrite($aFile, 'ES       :' . ES . ES_SYTEXTEOF  );
        ...
        fwrite($aFile, str_pad('',60,'-') . ES_SYTEXTEOF );
        fwrite($aFile, 'USER AGENT:' . $_SERVER['HTTP_USER_AGENT'] . ' (' . $_SERVER['REQUEST_METHOD'].')'. ES_SYTEXTEOF );
