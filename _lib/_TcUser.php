...
class TcUser
{
  ...
  function login($aUser, $aPass, $aRelogin = false)
  {
    ...
    $aQuery = "select idUser, name, code, password, SocialName, avatar from cl_user " .
    ...
    if ($this->query($aQuery) == 0) {
      ...
      if ($aRow = $this->FQuery->fetchRow()) {
        ...
        if (strcmp(trim($aRow['password']), trim(password($aPass))) == 0) {
          ...
          $_SESSION['time'] = time();                // Horario do Ultimo login
  ...
  function active()
  {
    ...
    if (isset($_SESSION['user'])) {
      ...
      if ($this->FDebugLevel[2]) $this->logDebug(__METHOD__);

      // Verificar se o IP é o MEsmo
      if (!($_SESSION['ip'] == get_IP())) {
